#!/usr/bin/env python3
"""
Manga/Manhwa Recap Video Generator (Ultimate Hybrid Version)
------------------------------------------------------------
Features:
  - Hybrid Audio: Custom voiceover (voiceover.mp3/wav/m4a) OR AI TTS fallback.
  - Script Generation from raw story via LLM (optional)
  - TTS via Sarvam AI (Hindi/English) with OpenAI fallback
  - Whisper API for exact timestamps + Auto-Subtitles (SRT)
  - Gemini Smart Sync to assign images to audio segments
  - Dynamic Ken Burns effects (12+ variations, orientation-aware, non-repeating)
  - Background music mixing
  - Final 1080p MP4 with burned subtitles
"""
...
if not OPENAI_API_KEY or not GEMINI_API_KEY:
    raise ValueError("Bhai, .env file mein OPENAI_API_KEY aur GEMINI_API_KEY zaroor daalna!")
...
INPUT_PANELS_DIR = BASE_DIR / "input_panels"
...
def ensure_dirs() -> None:
    """Create necessary directories and clean temp workspace."""
    for d in [INPUT_PANELS_DIR, TEMP_DIR, OUTPUT_DIR]:
        d.mkdir(parents=True, exist_ok=True)
    for f in TEMP_DIR.glob("*"):
        if f.is_file():
            f.unlink()
    logger.info("✅ Directories ready & temp cleaned.")
...
# ----------------------------
# Ken Burns Effects (12+ variations)
# ----------------------------
EFFECTS = [
...
class ImageProcessor:
    ...
    def generate_zoompan_filter(self, effect: str, duration: float, fps: int) -> str:
        ...
        if effect == "zoom_in":
            ...
            z = "1.2"
            x = "(iw-iw/zoom)/2"
            y = f"min( (ih-ih/zoom)*(on/{total_frames}) , ih-ih/zoom )"
...
def assemble_final_video(sync_data: List[Dict], audio_path: Path, bgm_path: Optional[Path], output_path: Path) -> None:
    ...
    logger.info("🔥 Final render (subtitles burn) chal raha hai...")
...
# ----------------------------
# Main Pipeline with Hybrid Audio
# ----------------------------
def main():
    ensure_dirs()

    prompts_file = BASE_DIR / "prompts.txt"
    bgm_file = BASE_DIR / "bgm.mp3"
    ...
    # Step 2: HYBRID AUDIO LOGIC
    # Pehle user ka custom audio dhundho
    custom_audio_found = False
    for ext in [".mp3", ".wav", ".m4a"]:
        possible = BASE_DIR / f"voiceover{ext}"
        if possible.exists():
    ...
    if not custom_audio_found:
        # No custom audio -> try script.txt
        script_file = BASE_DIR / "script.txt"
        if script_file.exists():
            logger.info("🤖 Custom audio nahi mila, script.txt se AI voice ban rahi hai...")
            narration_text = script_file.read_text(encoding="utf-8")
            generate_tts(narration_text, audio_file)
        else:
            # No script.txt -> try story.txt
            story_file = BASE_DIR / "story.txt"
            if story_file.exists():
                logger.info("📖 script.txt nahi mila, story.txt se script generate karke TTS kar rahe hain...")
                gen = ScriptGenerator(provider="gemini")
                narration_text = gen.generate_full_script(story_file.read_text(encoding="utf-8"))
                generate_tts(narration_text, audio_file)
            else:
                raise FileNotFoundError("Na custom audio (voiceover.mp3) hai, na script.txt, na story.txt! Kuch to do.")

    # Step 3: Whisper timestamps
    segments = get_whisper_segments(audio_file)

    # Step 4: Gemini smart sync
    ...
    logger.info("✅ Workflow complete! Video output folder mein check karo.")
